...
#define JOINT_MIN	-0.75f   //-0.75
#define JOINT_MAX	 2.0f   // 2.0
...
/* ADDED FOR EASY REFERENCE FROM DQN.py
...
#define LEARNING_RATE 0.1f   //0.01
#define NUM_ACTIONS 4            //6 because we have 3DOF(From AarmPlugin.h and each action can be +/-)
...
#define REWARD_GROUND -100.0f
#define REWARD_OTHERCOLL -25.0f
...
#define LOCKBASE true  //false

// Win only with gripper touch
#define GRIPPER_ONLY false  //false
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	{ 
		//if the object is colliding with the ground, ignore it. We check for that elsewhere anyway.
		if( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_GROUND) == 0 )
			continue;
...
#if GRIPPER_ONLY			        
                                rewardHistory = REWARD_OTHERCOLL - avgDist * 100.0f; // *100 worked well
#else                                
                                rewardHistory = REWARD_WIN;
#endif
...
bool ArmPlugin::updateAgent()
{
	...
	/         No Action           Joint 0         Joint 1          Joint2
        /Action       0            0 inc   1 dec    2 inc  3 dec    4 inc  5 dec
        */
	...
	/ 
	/ TODO - Set joint position +/-=actionJointDelta based on whether action is even or odd. 
        / 
        /         No Action           Joint 0         Joint 1          Joint2
        /Action       0            0 inc   1 dec    2 inc  3 dec    4 inc  5 dec
        */
...
// Determine if Gripper is in Collision or not
static bool checkGroundContact(const math::Box& a, const float groundLevel)
{

	if( a.min.z < groundLevel )
	{
		return true;
	}
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/
		...
		if(checkGroundContact(gripBBox, groundContact))
		{
			...
			rewardHistory = REWARD_GROUND - avgDist * 10.0f;
			newReward     = true;
			endEpisode    = true;
		}
		...
		*/ 
		...
		if(!checkGroundContact(gripBBox, groundContact))
		{
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = avgGoalDelta * REWARD_CLOSER;
				
				avgDist = (avgDist * alpha) + (distGoal * (1.0f - alpha));
				//rewardHistory = (-log(avgDist) + (avgGoalDelta * 10.0f) ) * REWARD_CLOSER;
				
				/*
				if (avgGoalDelta > 0.02f){
				        rewardHistory = REWARD_CLOSER + 10* REWARD_CLOSER/distGoal;
				}
				else{
				        rewardHistory = REWARD_FARTHER + REWARD_FARTHER * distGoal;
				}
				*/
				newReward     = true;	
			}
	...
	if( newReward && agent != NULL )
	{
		...
		if( endEpisode )
		{
			...
			outputFile.open("DQNResults.csv", std::ios::out | std::ios::app);
			outputFile << successfulGrabs << "," << totalRuns << "," << rewardHistory << "\r\n";
			outputFile.close();   
